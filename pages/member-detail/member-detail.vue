	...
	<view class="content">
		...
		<view class="ryxq" v-if="showxq!=0">
			...
			<view class="xq-list">
				...
				<view class="items">身份证号：{{ryinfo.id_card||'未知'}}</view>
				<view class="items">车牌号：{{ryinfo.plate_number||'未知'}}</view>
				...
				<view class="items">来源地区：{{ryinfo.from_address|addres}}</view>
				<view class="items">春运出行方式：{{ryinfo.traffic.type|trafficType}}</view>
				<view class="items">车牌或列车号：{{ryinfo.traffic.car_plate||'未知'}}</view>
				<view class="items">春运日期：{{ryinfo.check_in_time||'未知'}}</view>
			</view>
			...
			<view class="xq-list">
				...
				<view class="items">记录体温：{{ryinfo.temperature}}</view>
			</view>
...
<script>
	...
	export default {
		...
		filters: {
			...
			addres(val) {
				return (typeof val === 'object' ? val.addressStr || val.street : val) || '未知'
			},
			formatTime(val) {
				if (!val) {
					return '无'
